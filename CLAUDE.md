...
## 앱 구조 — 단일 화면
- 모드 분리 없음. 통합 사이드바(280px) + 3열 에디터가 한 화면에 공존.
...
```
┌───────────────────────────────────────────────────┐
│ ☰ 백지테스트 메이커                 📁폴더변경 ⚙설정 │  ← 헤더 (1줄)
├──────┬────────────────────────────────────────────┤
│파일   │ 단원명: [수동태_________]  [A4 미리보기]     │  ← sticky 단원명
│목록   │ 편집        │ ANSWER    │ WORKSHEET        │  ← sticky 열 헤더
│ ☑☐   │─────────────┼───────────┼──────────────────│
│(출력  │ [에디터셀]  │ [프리뷰]  │ [프리뷰]          │  ← 30행 단일 스크롤
│ 선택) │ ...         │ ...       │ ...               │
├──────┤                                             │
│[A][W]│  ← 에디터 포커스 시 사이드바 자동 닫힘       │
│[출력]│    (왼쪽 세로 바 클릭으로 다시 열기)           │
└──────┴────────────────────────────────────────────┘
```
